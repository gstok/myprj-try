...
$(function(){
    ...
    if(isCard=='?isCard'||type==1){
        $('.flashSMStitle').html('名片名称<font color="#FF0000">*</font>');
        vm.flashType=1;

    }else{
         $('.flashSMStitle').html('弹信名称<font color="#FF0000">*</font>');
         vm.flashType=2;
    }
...
var vm = new Vue({
    ...
    data: {
        ...
        ideaTag:'',//内容标签
        cusPropertyArr:[],
        SOperationFlash:[],//弹信下面的操作
    },
    methods: {
        //弹信功能下面的操作
        power:function(){
            $.each(Global.selectFunction, function (h, k) {
                if (k.moduleCode == "flash") {
                    vm.SOperationFlash.push(k.permissionCode);
                }
            });
            //弹信--编辑功能
            if(hasPermission("page", "UPDATE")){
                $('.smsEdit').find('input').removeAttr("readonly");
                $('.smsEdit').find('textarea').removeAttr("readonly");
                $('.save').removeAttr("disabled");
            }else{
                $('.smsEdit').find('input').attr("readonly","readonly");
                $('.smsEdit').find('textarea').attr("readonly","readonly");
        ...
        savepage:function(){
            ...
            jslength();
            if(lastNum>118){
             dcrmAlertWarning('预计弹信内容已经超过59个汉字,请重新编辑');
             return false;
            }
        ...
        custPropertySure:function(){
            ...
            setTimeout(function(){
                console.log(this.cusPropertyArr)
                jslength()
                // 提示还能输入几个汉字
                var elseNum=parseInt((118-lastNum)/2)
                if(elseNum>=0){
...
$('.editor_sms').on('input',function(){
    ...
    //         }
            ...
            // //计算所有属性中实际需要的字符数
            // attrNum=getStringLen(str2)
            // //计算实际字符
            // lastNum=sum-attrSum+attrNum;
            // if(lastNum>118){
            //     $('.xcConfirm').remove();
            //     dcrmAlertWarning('您输入的汉字已经超过59个字符,请重新编辑');
            // }
