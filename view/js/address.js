...
var provinceList = [
   ...
   {name:'重庆', cityList:[
      {name:'重庆市', areaList:['万州区','涪陵区','渝中区','大渡口区','江北区','沙坪坝区','九龙坡区','南岸区','北碚区','万盛区','双桥区','渝北区',
         '江津市','合川市','永川市','南川市','巴南区','黔江区','长寿区','綦江县','潼南县','铜梁县','大足县','荣昌县','璧山县','梁平县','城口县','丰都县','垫江县','武隆县','忠　县',
         '开　县','云阳县','奉节县','巫山县','巫溪县','石柱土家族自治县','秀山土家族苗族自治县','酉阳土家族苗族自治县','彭水苗族土家族自治县']},
   ]},
   {name:'四川', cityList:[
   ...
   {name:'海南', cityList:[
      ...
      {name:'三亚市', areaList:['崖州区','天涯区','吉阳区','海棠区区']},
      {name:'省直辖县级', areaList:['五指山市','琼海市','儋州市','文昌市','万宁市','东方市','定安县','屯昌县','澄迈县','临高县','白沙黎族自治县','昌江黎族自治县','乐东黎族自治县','陵水黎族自治县','保亭黎族苗族自治县','琼中黎族苗族自治县','西沙群岛','南沙群岛','中沙群岛的岛礁及其海域']}
   ]},
   {name:'台湾', cityList:['台北市','台中市','桃园市','台南市','高雄市','新北市']},
   {name:'香港', cityList:['香港岛','九龙半岛','新界']},
   {name:'澳门', cityList:[]}]
//多选
// var str='';
...
//     }
//     $('#province').html(str);
//     var content=$('#province').parent().find('.multiselect dropdown-toggle btn btn-default').attr('title');

//     //省份改变时，获取市区
//     $("#province").on("change", function () {
//        var _this = $(this);
//        var citycode = _this.val() || [];
//        var string=citycode.join(",");
//        var arr=string.split(',');//获取选中的值
//        var cityList='';
//        for(var i=0;i<arr.length;i++){
//             for(var j=0;j<provinceList.length;j++){
//                 if(arr[i]==provinceList[j].name){//如果选中的值和json中的值相同，就获取相应的城市
//                     for(var k=0;k<provinceList[j].cityList.length;k++){
//                         cityList+='<option value="'+provinceList[j].cityList[k].name+'">'+provinceList[j].cityList[k].name+'</option>'
//                     }
//                 }
//             }
//        }
...
//        for(var i=0;i<arr.length;i++){
//             for(var j=0;j<provinceList.length;j++){
//                 //console.log(provinceList[j])
//                 for(var m=0;m<provinceList[j].cityList.length;m++){
//                     //console.log(provinceList[j].cityList[m])
//                     //console.log(arr[i])
//                     if(arr[i]==provinceList[j].cityList[m].name){//如果选中的值和json中的值相同,就获取相应的县区
//                         var countyArr=provinceList[j].cityList[m].areaList;
//                         for(var k=0;k<countyArr.length;k++){
//                            // console.log(countyArr)
//                            countyList+='<option value="'+countyArr[k]+'">'+countyArr[k]+'</option>'
//                         }
//                     }
//                 }
//             }
