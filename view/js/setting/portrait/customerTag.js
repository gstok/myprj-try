...
// 客户标签列表
function custTagList(page,size,count){
	var data = {
        "pageIndex":page,
        "pageSize":size,
        "type":0
    ...
    MaskUtil.Loading();
	API.custTagList(data,function(data){
		console.log(data);
        ...
        var statusName;
        var createUserId=obj[i].createUserId;
        var bjsc;
        if(status!=2){
        	if(createUserId=='华坤'){
        		bjsc='';
	        }else{
	        	if(hasPermission("custTag","UPDATE")){
		        	bjsc='<a href="javascript:;" class="tbBtn zcBtn" onclick="editCustomerTag(event,'+obj[i].id+')">编辑</a>'+
		            '<a href="javascript:;" class="tbBtn zcBtn" onclick="delCustTag(event,'+obj[i].id+',\''+obj[i].tagName+'\')">删除</a>'
	        	}else{
...
function addsure(){
	...
	if(data.tagName.length>5){
		...
		dcrmAlertError('标签名不能为空');
		return false;
	}
	//MaskUtil.Loading();
	API.addCustTag(data, function(res) {
		console.log(res);
		if(res.code == 200){
...
// 删除客户标签
function delCustTag(event,id,name){
	event.stopPropagation();
  	var data = {
  		id:id
  	};
  	dcrmConfirm("您确定要删除客户标签'"+name+"'吗？",function(){
	  	MaskUtil.Loading();
