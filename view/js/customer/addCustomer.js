...
// 新建或编辑客户展示
	...
	API.custSelectById(data,function(data){
		...
		if(data.data.commonAttrList.length){
			$.each(data.data.commonAttrList,function(i,u){
				if(u.attrKey !='wx_head_img_uri'){
					arr.push(u)
				}
			})
			for (var i =0;i< arr.length; i++) {
				...
				if(arr[i].attrType=='date'){
				   typeList=' <input type="text" value="'+contentvalue+'" class="wicon" id="'+arr[i].attrKey+'" placeholder="选择日期" readonly >';
					...
					if(arr[i].attrKey=='mobile'){
						if(contentvalue.indexOf("*") > 0){
							window.mobileNum = contentvalue;
							typeList='<input type="text" placeholder="该手机号已加密" id="'+arr[i].attrKey+'" maxlength="11" onkeyup="this.value=this.value.replace(/\\D/g,\'\')">';
						}else{
							typeList='<input type="text" maxlength="11" onkeyup="this.value=this.value.replace(/\\D/g,\'\')" value="'+contentvalue+'"  id="'+arr[i].attrKey+'">';
						}
					}else{
		...
		if(arr2.length){
			for (var i =0;i< arr2.length; i++) {
				...
				}else if(arr2[i].attrKey=='telephone'){
						typeList2='<input type="text" maxlength="13" value="'+contentvalue2+'" onkeyup="this.value=this.value.replace(/[^0-9,^-]/g,\'\')"  id="'+arr2[i].attrKey+'">';
				}else{
...
function addSure(){
	...
	API.custSelectById(data,function(data){
		...
		if($('#telephone').length>0){
	    	...
	    	if($.trim(telephone)!=''&&!(/^\d{3,4}-?\d{7,9}$/.test(telephone))){
	       		dcrmAlertError("联系电话有误，请重填");
	        	return false;
			}
		}
		if($('#mobile').length>0 && $('#telephone').length>0){
		 	...
		 	var telephone = $('#telephone').val();
	    	if($.trim(phone)!=''&&($.trim(phone) == $.trim(telephone))){
	       		dcrmAlertError("手机号码跟联系电话不能重复，请重填");
	        	return false;
			}
		 }
