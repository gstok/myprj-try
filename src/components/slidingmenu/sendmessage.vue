<template>
    <Modal v-model="pushMessage" width="560" :mask-closable="false" :transfer="false" @on-cancel="sendCancel">
        ...
        <div class="selectinfor">
            <p class="Prompt">
                ...
                <span class="many"> {{ emptyMobile }} </span>个客户手机号为空将无法发送短信！
            </p>
            <div class="peoplelist">
                <div class="infor" v-show="firstshow">
                    <span v-for="(people,index) in sendMsgCustHave" v-if="index < 20" :key="people.id" class="spancla">
                        {{ people.name }}
                        <Icon type="close-round" @click.native="removeCust(index,people.id,1)" title="删除" style="font-size: 10px" v-show="showRemoveCustIcon"></Icon>
                    </span>
                    <span v-for="(people,index) in sendMsgCustHasNo" v-if="index < (20-sendMsgCustHave.length)" :key="people.id">
                        {{ people.name }}
                        <Icon type="close-round" @click.native="removeCust(index,people.id,2)" title="删除" style="font-size: 10px" v-show="showRemoveCustIcon"></Icon>
                    </span>
                </div>
                ...
                <div class="btncon">
                    ...
                    <Button type="text" size="small" style="clear:both;" @click="seeMore" v-show="btnshow1">
                        <Icon type="chevron-down"></Icon>&nbsp;&nbsp;查看更多
                    </Button>
                    <Button type="text" size="small" style="clear:both;" @click="shouqi" v-show="btnshow2">
        ...
        <Form class="templinfor" :label-width="80">
            <FormItem label="短信模板" prop="mestype">
            ...
            <FormItem>
                <Input v-model="mestype" type="textarea" :rows="4" readonly placeholder="请输入您要发送的信息..." />
            </FormItem>
        </Form>
        <div slot="footer">
...
export default {
            ...
            if (val === true) {
                this.$post(this.global.baseUrl + '/pc/newCust/getMoreList.action', this.custSearchParams).then(res => {
                    if (res.code === 200) {
                        ...
                        //this.sendMsgCust = res.data.data;
    ...
    created() {
        // 发短信模板
        this.$post(this.global.baseUrl + '/pc/sms/getSmsList.action').then(res => {
            if (res.code === 200) {
    ...
    methods: {
        sendmesOk() {
            ...
            this.$post(this.global.baseUrl + '/pc/sms/newSmsBatchSend.action', params).then(res => {
                if (res.code === 200) {
