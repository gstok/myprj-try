<template>
    <Modal v-model="pushMessage" width="560" :mask-closable="false" :transfer="false" @on-cancel="sendCancel">
            ...
            <div class="peoplelist">
                ...
                <div class="infor" v-show="secondshow">
                    <span v-for="(people,index) in sendMsgCustHave" :key="people.id" class="spancla">
                      {{ people.name }}
                      <Icon type="close-round" class="icon icon-cha" @click.native="removeCust(index,people.id,1)" title="删除" style="font-size: 10px"
                         v-show="showRemoveCustIcon"></Icon>
                    </span>
        ...
        <Form class="templinfor" :label-width="80">
            <FormItem label="弹信模板" prop="mestype">
...
<script>
    export default {
        ...
        created() {
            this.messPeople2 = [];
            // 发短信模板
            this.$post(this.global.baseUrl + '/pc/flashSms/getFlashList.action').then(res => {
                if (res.code === 200) {
        ...
        methods: {
            sendmesOk() {
                ...
                this.$post(this.global.baseUrl + '/pc/flashSms/newFlashBatchSend.action', params).then(res => {
                    if (res.code === 200) {
                        this.$Message.success(res.msg);
                        this.$emit('onsuccessTan');
                    }
                });
            },
            sendCancel() {
                this.$emit('onerrorTan');
            },
